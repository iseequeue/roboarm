...
)
{
    ...
    */
    for (size_t i = 0; i < _primitiveSteps.size(); ++i) {
        JointState nextState = node->state() + _primitiveSteps[i];

        // Check if nextState is a valid state (i.e., not in collision).
        if (!checkCollisionAction(node->state(), _primitiveSteps[i]) && nextState.isCorrect()) {
            // Compute the cost to reach this successor node from the start.
            CostType cost = node->g() + costMove(node->state(), nextState);

            // Compute the heuristic cost from this successor node to the goal.
            CostType heuristic = (*heuristicFunc)(nextState, goal)*weight;

            // Create the new successor node.
            astar::SearchNode* successor = new astar::SearchNode(cost, heuristic, nextState, i, node);

            // Add the new successor node to the list of successors.
...
)
{
    ...
    while (currentNode != nullptr)
    {
        ...
        */
        ...
        {
            astar::SearchNode* successor = successors[i];

            // Если сосед уже в CLOSED, пропускаем его.
        ...
        // Добавляем текущий узел в CLOSED.
